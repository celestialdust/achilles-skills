...
- **Boundary.** `explain` makes the *human* understand — not `code-review` (which judges a diff for merge) and not `codebase-research` (a goal-blind survey feeding the plan). See the skill's when-to-use table.
