---
description: "One-time repo bootstrap — scaffold the shared substrate every achilles skill assumes: the STATE.md board, the CONTEXT.md glossary, docs/adr/, docs/features/, docs/session-state.md, docs/progress.md, docs/lessons.md, and the `## Agent skills` block in one of CLAUDE.md / AGENTS.md plus a short pointer to it in the other."
---
...
- Scaffolds the substrate — `STATE.md` (empty two-level board), `CONTEXT.md` (`## Glossary` stub), `docs/adr/`, `docs/features/`, `docs/session-state.md` (where the work stands, plus an append-only log of decisions a resuming session reads first), `docs/progress.md` (the run record — what each slice actually executed, seeded with the entry shape and no entries), `docs/lessons.md` (the lessons record — what a root-caused defect turned out to be and the guard that would catch it coming back, seeded with the field template and no entries), and the `## Agent skills` block in `CLAUDE.md` or `AGENTS.md`. No values, secrets, or commands; no feature/slice rows, no log entries.
