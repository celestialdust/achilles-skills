...
1. **shipping-and-launch** — run the pre-launch checklist, plan the staged rollout, and write the rollback plan into `release.md`. This is the release discipline: confirm the slice is launch-ready before any PR opens. It runs first because step 2 anchors the PR to the rollback plan, so the plan has to exist by then.
...
- Ship *writes* the release runbook; it never deploys. The deploy, the staged rollout, and any rollback are steps the human runs after merging. The agent's span ends at the open draft PR. So nothing in this stage waits on a merge — a merge is not something the agent can act on.
