...
Review → Ship **fully autonomously with no mid-run human halt** (D29), `acceptance.md` is the **SOLE
...
It is a **prose contract, not a test framework** (D12): no Cucumber, no step-defs, no `.feature` engine.
...
- you are tempted to write a **design** scenario ("the button is blue", "the modal slides in", "the focus
  ...
  `frontend-design`'s signed design contract (D20). Zero design content enters `acceptance.md`. The two
...
4. **Id every scenario.** Use the feature's PRD namespace + `A` + number: `PWR-A1`, `PWR-A2`, …
   (matching the STATE.md PRD-namespacing, D13). Tag each scenario with the **story id it realizes**
...
7. **Present for sign-off.** Hand the human a complete draft. On their sign, `status → draft` becomes
   ...
   `acceptance.md` to `draft`** (D20) — the contract must be re-signed if the product spec moves.
...
- **NO engine** — no `.feature` files, no `@given`/`@when` step definitions, no Cucumber/Behave/SpecFlow.
  Prose only; `test-driven-development` turns prose into tests by judgment (D12).
...
- "This UI scenario needs the button colour / the modal animation." → No. Design lives **wholly** in
  `frontend-design`'s contract (D20). Behavioral-only here — assert what the user *does*, not how it looks.
...
- "It looks right — I'll mark it `signed`." → No. The **human signs**; `signed` set by the agent forges
  the sole human-anchored oracle (D29). Leave it `draft` and present for sign-off.
- "Cucumber would make this executable." → No engine (D12). Executability comes from `test-driven-development` realizing the
...
- **Coverage:** every numbered id in `prd.md`'s `## User Stories` is named by ≥1 scenario's `realizes:`
  tag. (This is the D29 completeness invariant `spec-review` re-checks: every story id → ≥1 reachable
  exercised scenario.)
...
- **Downstream consumers:** `test-driven-development` (realizes each scenario as a RED test, test-first; refuses an
  ...
  reachable scenario before the human reviews, D22) · `pull-request` (any "not-reachable" classification at run time
  → a required human-ack line in the PR body, never silently absorbed, D29).
- **Frozen-under-retry invariant (D29):** once signed, `acceptance.md` is **immutable during a slice's
...
- **Boundary with `frontend-design`:** design floors/rubric/prototype-fidelity live wholly in its signed
  ...
  content (D20). No two signed artifacts can contradict.
