...
|---|---|---|---|
| `plan.md` + `plan/<slice-id>.md` | `plan-breakdown` | the assigned slice's row keyed by **Slice id** — its **Story-ref · Design ref · Steps · Files (owned) · Regression surface · Checkpoint · Blocked-by** columns — plus the line-level steps and exact tests in the file the `Steps` cell names, `docs/features/<slug>/plan/<slice-id>.md` | no `plan.md`, or no step file for the assigned slice |
