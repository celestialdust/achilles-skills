 ...
 * judgment + the brief (docs/design.md, the feature's prd.md, the user's
 ...
 *   node scripts/palette.mjs --id seed-000    # pick a specific seed
 *   node scripts/palette.mjs --from <key>     # hash <key> to a seed (deterministic)
 *
 * Env vars:
 *   BRAINSTORM_PALETTE_SEED — same as --from; makes a run reproducible.
 *
 * Self-contained: reads no file, opens no socket, and imports nothing
 * outside node: builtins. Anything it tells the caller to apply is printed
 * below, not cited to another document.
 */
...
function pickSeed(seeds, { id, from }) {
  ...
  const envFrom = process.env.BRAINSTORM_PALETTE_SEED;
...
the palette around it using YOUR judgment, the brief (docs/design.md, the
feature's prd.md, the user's prompt), and the color-strategy guidance
printed below.
...
4. Pick a color STRATEGY (four registers, pick one):
...
Hard rules (stated here because the seed step is where they actually bite):
