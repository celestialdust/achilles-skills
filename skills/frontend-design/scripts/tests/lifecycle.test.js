...
const START = path.join(__dirname, '../start-server.sh');
const STOP = path.join(__dirname, '../stop-server.sh');
...
function newestSessionDir(projectDir) {
  const sessionDir = execFileSync('bash', [
    '-lc',
    'find "$1/.frontend-design" -mindepth 1 -maxdepth 1 -type d -print | sort | tail -1',
    'bash',
    projectDir
  ], { encoding: 'utf8' }).trim();
  assert(sessionDir, `expected at least one session dir under ${projectDir}/.frontend-design`);
  return sessionDir;
}
